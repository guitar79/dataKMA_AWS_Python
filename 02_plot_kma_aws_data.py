...
이제 timestamp가 들어 있는 ['dt_YmdHM'] 컬럼을 index로 지정하는 것이 좋겠다. 
"""
...
while start_date < (max(df['dt_YmdHM-1min'] - timedelta(days = 27))) :
    ...
    plt.legend(lns, labs)
    plt.savefig("{}{}_{}.png".format(base_dr, start_date.strftime('%Y%m%d'), end_date.strftime('%Y%m%d')))
